<template>
	<scroll-view :scroll-y='true' class="con" :show-scrollbar='true' ref='scrollView' :scroll-top="scrollTop">
		<movable-area class="area" :style="{ height: areaHeight+'px'}" @mouseenter="mouseenter" @mouseleave="mouseleave">
			<block v-for="(item, index) in imageList" :key="item.id">
				...
				:damping="40" 
				:disabled="item.blob==undefined?false:true" 
				...
				>
					<view class="area-con" :style="{ width: childWidth, height: childWidth, transform: 'scale(' + item.scale + ')' }">
						...
						<view class="wait-upload-mask" v-if="item.blob != undefined">
							<text class="iconfont icondengdaishangchuan1"></text>
							<text>等待上传</text>
						</view>
					</view>
				</movable-view>
			...
			<view class="add" v-if="imageList.length < number" :style="{ top: add.y, left: add.x, width: viewWidth + 'px', height: viewWidth + 'px' }"
			 @click="addImages">
				<view class="add-wrap" :style="{ width: childWidth, height: childWidth }">
					<image style="width: 54rpx;height: 54rpx;" src="data:image/png;base64,"></image>
				</view>
			</view>
		</movable-area>
	</scroll-view>
	...
	export default {
		...
		computed: {
			areaHeight() {
				if (this.imageList.length < this.number) {
					return Math.ceil((this.imageList.length + 1) / this.colsValue) * this.viewWidth
				} else {
					return Math.ceil(this.imageList.length / this.colsValue) * this.viewWidth
				}
			},
			childWidth() {
		...
		methods: {
				...
				if (this.custom) {
					...
					uni.chooseImage({
						...
						success: res => {
							...
							// }
							res.tempFiles.forEach(item => this.addProperties(item))
						}
					})
				}
			},
			...
			sortList() {
				...
				this.$emit('updateList', list, this.$refs.scrollView)

				this.$nextTick(() => {
					uni.createSelectorQuery().select(".area").boundingClientRect((res) => {
						this.scrollTop = res.height
					}).exec()
				})
			},
			updatePath(path,index){
				this.imageList[index]['src'] = `https://${path}`;
				this.imageList[index]['blob'] = undefined;
			},
			addProperties(item) {
